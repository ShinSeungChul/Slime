# SlimeUnity
<img src="/Assets/Resources/#Images/icon.PNG" width="250" height="250" align="center"/>

## 앱 소개

### Unity를 사용하여 C#으로 개발한 킬링 타임용 모바일 게임

* 지하철, 버스 등의 이동 중 간단히 플레이할 수 있습니다.
* 과금을 요구하지 않고 게임을 플레이하며 **Coin**을 얻거나, 미션을 클리어하여 Slime을 꾸밀 수 있습니다.
* 애니메이션을 추가하여 슬라임들의 간단한 배경을 알 수 있습니다.
* 좌뇌와 우뇌를 동시에 사용하는 게임이므로 굉장히 어렵고, 두뇌 활성화에 도움을 줍니다.
* 도트 디자인으로 레트로 느낌이 납니다.

- - -

### 인게임 UI

<table>
  <tr>
    <th align="center">
       <img width="200" alt="1" src="https://user-images.githubusercontent.com/11826495/79063466-d8319980-7cdc-11ea-9037-5adc017e8300.gif"/>
       <br><br>[Change Hat] 
    </th>
     <th align="center">
      <img width="200" alt="1" src="https://user-images.githubusercontent.com/11826495/79063498-0616de00-7cdd-11ea-922e-59e584736025.gif"/>
       <br><br>[Clear Mission]
    </th>
     <th align="center">
       <img width="200" alt="1" src="https://user-images.githubusercontent.com/11826495/79063165-ed0d2d80-7cda-11ea-89e2-cc625d6eb522.gif"/>
       <br><br>[Jump]
     </th>
     <th align="center">
      <img width="200" alt="1" src="https://user-images.githubusercontent.com/11826495/79063296-a7049980-7cdb-11ea-9940-14bb0eb48014.gif"/>
       <br><br>[Moving Thorn]
    </th>
  </tr>
</br>
  <tr>
      <th align="center">
       <img width="200" alt="1" src="https://user-images.githubusercontent.com/11826495/79063092-8a1b9680-7cda-11ea-875e-d82b9a100f8f.gif"/>
       <br><br>[Switch] 
    </th>
     <th align="center">
      <img width="200" alt="1" src="https://user-images.githubusercontent.com/11826495/79063191-247bda00-7cdb-11ea-8c13-ddfac387e7cc.gif"/>
       <br><br>[Reverse]
    </th>
 <th align="center">
       <img width="200" alt="1" src="https://user-images.githubusercontent.com/11826495/79063355-2befb300-7cdc-11ea-943c-dc1bb4a3ad1f.gif"/>
       <br><br>[Select Stage]
     </th>
     <th align="center">
      <img width="200" alt="1" src="https://user-images.githubusercontent.com/11826495/79063338-fd71d800-7cdb-11ea-890a-b5b673fc1249.gif"/>
       <br><br>[Death]
    </th>
  </tr>
</table>

- - -

### Idea

* 당시 Steam의 항아리 게임, 고양이 마리오 등의 어렵고, 상상하지 못한 곳에서 장애물이 나오는 게임이 유행
* 오마쥬를 하여 어려운 난이도를 설정했고, 나도 모르는 곳에서 튀어 나오는 장애물들을 구현했습니다.

#### 좌뇌와 우뇌의 사용을 이용한 차별성

* 동시에 왼손과 오른손을 함께 이용해야해서 좌뇌와 우뇌 모두 발달하고 사용하는 효과를 얻습니다.
* 또한 포탈을 통해 슬라임의 위치 또한 바뀌기 때문에 그 어려움은 극에 달합니다.

- - -

### Issue

* 구현은 지속적으로 논리식과 물리식을 수정하며 개발했습니다.
* 하지만 게임의 얼굴이라 불리는 디자인이 문제였습니다.

#### 주변의 지속적인 응원과 피드백

* 간단한 디자인(명암, 채도 등)에 대해 공부했습니다.
* 파랑과  빨강의 컨셉을 살리며 만족스러운 디자인 완성.
 
- - -

### UI

<img src="/img/slime_main.jpg" width="360px" height="480px" title="메인화면" alt="Main"></img>
<img src="/img/slime_mission.jpg" width="360px" height="480px" title="미션팝업" alt="Mission"></img></br>
<img src="/img/slime_store.jpg" width="360px" height="480px" title="상점화면" alt="Store"></img>
<img src="/img/slime_pause.jpg" width="360px" height="480px" title="일시정지팝업" alt="Pause"></img></br>
<img src="/img/slime_stage13.webp" width="360px" height="480px" title="Stage13" alt="Stage13"></img>
<img src="/img/slime_stage44.webp" width="360px" height="480px" title="Stage44" alt="Stage44"></img></br>

- - - 

### 시작 애니메이션

#### 클릭하면 실행됩니다.

[![Video Label](http://img.youtube.com/vi/w5hAHB-yZq4/0.jpg)](https://youtu.be/w5hAHB-yZq4?t=0s)

- - -

### 플레이 영상

#### 클릭하면 실행됩니다.

#### 포탈을 사용하여 슬라임의 위치를 변경합니다.
[![Video Label](http://img.youtube.com/vi/nCQ2HgwOrE0/0.jpg)](https://youtu.be/nCQ2HgwOrE0?t=0s)

- - -

#### 장애물이 움직입니다.

[![Video Label](http://img.youtube.com/vi/hFe28uKH7AI/0.jpg)](https://youtu.be/hFe28uKH7AI?t=0s)

- - -

#### 화살표에 닿으면 이동 방향이 변경됩니다.

[![Video Label](http://img.youtube.com/vi/3GdrOGBC7Tg/0.jpg)](https://youtu.be/3GdrOGBC7Tg?t=0s)

- - -

### 엔딩 애니메이션

#### 클릭하면 실행됩니다.

[![Video Label](http://img.youtube.com/vi/EMmTcVZMjn4/0.jpg)](https://youtu.be/EMmTcVZMjn4?t=0s)
